...
function check(cell) {
	return check_line(cell);
}

function check_line(cell) {
...
function get_row_group(row) {
    var cells = user_interface.cells;
    var row_cells = [];
    for (var i = 0; i < cells.length; i++) {
...
function remove_listener(elements, eventName, handler) {
    for (var i = 0; i < elements.length; i++) {
        x = elements[i];
        if (x.addEventListener) {
            x.removeEventListener(eventName, handler, false);
        } else if (x.detachEvent) {
            x.detachEvent('on' + eventName, handler);
        } else {
            x['on' + eventName] = null;
        }
    }
